    ...
    xhr.onload = (res) => {
        if(xhr.status == 204){   
            let checkElementExixt = document.getElementById(`DIV${deleteUserId}`);
              ...
              setTimeout(function(){ alert(`user number ${deleteUserId} has been successfully deleted`); }, 100); 
            }
    }
}
...
let getUsersAndRender =  (renderUsers) => {
    ...
    let theFullPath = window.location.href;
    const urlParam = new URLSearchParams(window.location.search);
    let URL;
    if(theFullPath.includes('/jslab51.html') || urlParam.get('link') == 'first'){
        
         URL = 'https://reqres.in/api/users';
    }else if(theFullPath.includes('/userspage2.html') || urlParam.get('link') == 'second'){
...
let renderUsersTOHTml= (users) => {    
    ...
    for (let i = 0; i < usersUrl.length; i++) {
        ...
        const deleteButton = document.createElement('button');
             ...
             parentDiv.innerHTML += `<a href="jslab51.html?link='first'">1</a>`+" ";
             parentDiv.innerHTML += `<a href="userspage2.html?link=second">2</a>`;
